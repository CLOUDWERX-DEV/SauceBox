...
- **Vertical Centering Alignment**: Removed the `opacity: 0.5` washed-out style from empty state icons to ensure they look vibrant and rich. Added `contentContainerStyle={{ flexGrow: 1 }}` to both available videos ScrollViews so they stretch and center the empty state components perfectly vertically in their panel container.
