...
### Added
- **Beautiful Active Playlist Name Header Badge with Auto-Truncation**: Added a visually stunning, custom-styled Active Playlist Name Badge (`🎬 Playlist: [Name]`) to the built-in video player (`VideoPlayer.js`). When a video is launched with saved playlist context, this high-contrast, premium badge is automatically rendered in the header metadata row. Styled with an ultra-subtle theme orange wash (border `#FF8C0080`, background `#FF8C0015`, and bold text `#FF8C00`) to stand out. It dynamically truncates long playlist names exceeding 20 characters to prevent header wrapping layout issues, while wrapping the badge in the modular `<Tooltip>` component to present the complete, untruncated playlist name upon hover!
- **Interactive Prev/Next Navigation Context in Gallery and Playlist Editor**: Added full playlist previous/next navigation context when playing individual videos directly from the main Gallery Tab (`GalleryTab.js`) and the Playlist Editor view (`PlaylistsTab.js`). When a user plays a video, we dynamically construct a playlist context from the currently filtered/sorted gallery items (or the playlist draft items), pre-resolve paths, and inject the current item's relative index into the Zustand built-in player active payload. The video player automatically renders the premium `⏮ Prev` and `Next ⏭` header navigation buttons, allowing seamless cycling through the user's active filter states.
- **Premium Video Player Metadata Badges Tooltips**: Wrapped all metadata badges inside the built-in video player (`VideoPlayer.js`) in our modular `<Tooltip>` component. Hovering over duration (`⏱️`), resolution (`📺`), file size (`💾`), and track position (`🗂️`) now presents highly descriptive tooltips (`Duration`, `Resolution`, `File Size`, `Track Position in Playlist`) with bottom alignment. Established a proper stacking context (`position: 'relative', zIndex: 10`) on the parent badge row container to prevent stacking conflicts with the trimmer panel and video tag wrapper.
- **Premium Tooltips for Metadata Badges**: Added our modular custom `<Tooltip>` component to each metadata badge across the Gallery, Queue, and Playlist tab cards. Users now get a sleek hover tooltip explaining exactly what each statistic represents (e.g. `Duration`, `Resolution`, `File Size`, `Date Added`, `Queued`, `Video Count`, `Total Duration`, `Total Size`, `Date Created`) using index-aware positioning for top-tier visual hierarchy.
- **Premium SauceBox Hub & Ecosystem Documentation Section**: Designed and integrated a gorgeous, premium "SauceBox Hub" section into the documentation modal. Features a conversion-optimized support card highlighting CLOUDWERX LAB's open-source philosophy, featuring a three-column donation tier grid (Coffee, Beer, Legend) and a high-impact primary orange Support button. Also integrates an interactive grid of official community resources (linking to `saucebox.app`, `cloudwerxlab.com`, and our active GitHub repository/profile) and a detailed engines card honoring our powerful open-source foundation (`yt-dlp` and `ffmpeg`).
...
### Fixed
- **Resolved Video Player Tooltip Cutoff Bug**: Fixed a stacking context bug where downward-floating tooltips in the built-in video player (`VideoPlayer.js`) were being cut off and painted underneath the trimmer control panel and video element. Applied `position: 'relative'` and `zIndex: 100` to the player's `header` stylesheet block. This guarantees the entire header establishes a superior stacking context above all subsequent siblings inside the player container, letting the tooltips render beautifully on top.
...
### Changed
- **Optimized Navigation Sidebar Layout & Spacing**: Balanced the sidebar layout proportions inside `Sidebar.js` to establish a perfect middle ground between vertical economy and aesthetic fullness. Shrunk the logo icon to `56x56` (with `padding: 20` and `marginBottom: 10` on the header), set the tab container padding to `18`, tab item paddings to `14`, tab icon sizes to `25`, and footer padding to `18`. This configuration saves **58px of total height**, ensuring all tab selections, badges, stats, and donation buttons fit perfectly with zero scrollbar trigger at default window height, while leaving a clean, compact, and balanced gap above the footer.
...
- **Increased Queue Cards Thumbnail Scale**: Upgraded the layout proportions of active/completed download cards inside the Download Queue tab. Increased the video thumbnail dimensions from `192x108` to `240x135` (retaining the exact, cinematic 16:9 aspect ratio) to balance the card layout visually and provide a significantly higher-impact, cleaner visual fit.
- **Redesigned Download Card Metadata Badges**: Extended the premium metadata chips component design to the Download Queue tab. Replaced the plain bullet-separated string on all active/completed download queue cards (`DownloadCard.js` / `QueueStyles.js`) with structured, glassmorphism-themed bubble chips: `⏱️ Duration`, `📺 Resolution`, and `💾 Filesize`, matching the premium design language established in the built-in video player.
- **Redesigned Video Player Metadata Header**: Fully replaced the boring plain-text bullet-separated video metadata string in the built-in video player's header with a row of beautiful, discrete, premium metadata chips. Structured them into individually styled bubble containers with semi-transparent white border washes and custom icon prefixes: `⏱️ Duration`, `📺 Resolution`, `💾 Filesize`, and a primary-tinted `🗂️ Track Index` when navigating through playlists.
...
- **Eliminated Forced Sidebar Scrollbar Track**: Swapped out React Native Web's default `<ScrollView>` component with a standard `<View>` styled container featuring a responsive CSS `overflowY: 'auto'` rule on the main app navigation sidebar (`Sidebar.js`) and the Help/Docs sidebar (`HelpModal.js`). This completely bypasses React Native Web's runtime wrapper styling which hardforces `overflow-y: scroll`, ensuring the left navigation panel is 100% clean and free of blank scrollbar tracks at the default window size, while maintaining full vertical scrollability when the window height is squeezed.
...
- **App-Wide Tooltip Alignment & Sizing Overhaul**: Completely refactored and streamlined tooltip descriptions across the entire application to use punchy, professional, single-word or highly compact labels. Converted `"Quick Stream / VR Cast"` to `"Quick Cast"`, `"Open Download Folder"` to `"Open Folder"`, `"Delete Video"` / `"Delete Playlist"` to `"Delete"`, and `"Remove from Playlist"` / `"Remove from Queue"` to `"Remove"`. This prevents horizontal text overflow clipping on narrow panels. Also removed the layout-level `overflow: 'hidden'` property from the left and right panels of both the Playlist Builder editor pane (`PlaylistStyles.js`) and the Broadcast playlist builder columns (`BroadcastPlaylistBuilder.js`) to guarantee that list reordering buttons (Move Up, Move Down, and Remove) never suffer clipping under any scale.
...
- **Expanded Application-Wide Tooltips**: Integrated modular, beautiful tooltips on the Playlist and Broadcast tabs to enhance interactive usability. Wrapped the two top-right action buttons (Quick Stream / VR Cast, Delete Playlist) on the Playlist gallery cards inside tooltips using secure bottom alignment (`position="bottom"`) to prevent rounded card `overflow: hidden` clipping. Wrapped all move up (`⬆️`), move down (`⬇️`), and remove (`❌`) buttons inside list elements on both the right-hand Playlist Builder editor panel (`PlaylistEditor.js`) and the Broadcast playlist panel (`BroadcastPlaylistBuilder.js`) with responsive top-aligned tooltips.
- **Interactive Tooltip System**: Engineered and introduced a production-grade, modular `Tooltip` component built natively for React Native Web, providing a standardized blueprint for elegant hover documentation across the app with support for custom alignment position parameters (top/bottom). Wrapped the top-right action buttons inside both the Download Queue cards ("Open Download Folder", "Remove from Queue" utilizing top alignment) and the Gallery cards ("Quick Stream / VR Cast", "Open Download Folder", "Delete Video" utilizing bottom alignment to prevent container `overflow: hidden` clipping), completing them with smooth absolute alignment, custom arrow indicators, deep charcoal background styling (`#111111`), and subtle orange borders (`#FF8C0040`) for an exceptionally premium aesthetic.
...
- **Tab Header Description Standardization**: Added premium, persistent description subtitles to the header sections of all main application tabs to ensure structural and visual consistency across the app. Subtitles include: `"Monitor, pause, and manage your active download streams ⏳"` on Queue, `"Search, rate, tag, and play your private offline media archive 🎥"` on Gallery, `"Build, sequence, and play custom video collections 🗂️"` on Playlists, and a refined `"Configure download directories, preferred quality, secure vaults, and system parameters ⚙️"` on Settings.
- **Premium Support Dashboard**: Completely redesigned and overhauled the Settings Support section into a highly premium, conversion-optimized donation dashboard. Added compelling, developer-direct copywriting emphasizing CLOUDWERX LAB's open-source philosophy, privacy pledge, and ongoing server/build workloads. Integrated an interactive three-column "Support Tiers" grid (Coffee, Beer, and Legend levels) complete with custom emojis, typography, and clear calls-to-action to maximize donor engagement and support.
