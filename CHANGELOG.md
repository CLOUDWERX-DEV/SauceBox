...
### Changed
- **Interactive Tooltip System**: Engineered and introduced a production-grade, modular `Tooltip` component built natively for React Native Web, providing a standardized blueprint for elegant hover documentation across the app. Wrapped the "Open Download Folder" and "Remove from Queue" action buttons inside the Download Queue card in this tooltip system, completing them with smooth absolute alignment, custom arrow indicators, deep charcoal background styling (`#111111`), and subtle orange borders (`#FF8C0040`) for an exceptionally premium aesthetic.
