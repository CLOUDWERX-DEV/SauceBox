...
### Fixed
- **Elevated Hovered Tooltip Stacking Order**: Fixed a rendering/stacking bug where bottom-aligned tooltips (such as the topmost queue card badge tooltip) drew underneath neighboring text blocks like `✔ Completed` due to layout stacking contexts. Applied `visible && { zIndex: 9999 }` to the Tooltip wrapper container so that any hovered tooltip instantly claims full stacking priority over neighboring layout items.
