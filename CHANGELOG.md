...
- **Top-Right Import Button Hiding**: Refactored the `GalleryHeader.js` layout to hide both the "📥 IMPORT VIDEOS" and "🗑️ Clear All" top-right buttons when the main Video Gallery database is completely empty. This removes the visually redundant button double-up since the central empty state already displays a giant, primary "📥 IMPORT VIDEOS" button.
