...
- **Eliminated Double-Scrollbar Glitch**: Locked the main outer page viewport wrapper (`main-content-wrapper`) to `overflow-y: hidden` inside `public/index.html`. Since every single tab now has its own internal `<ScrollView>` (including the newly wrapped `DownloadTab`), double-scrolling is completely abolished, guaranteeing that a scrollbar will **never** appear on the far right of the application page, ever.
- **Playlist Editor Dynamic Window Sizing**: Redesigned the Edit Playlist page layout to stretch and size dynamically with the window. Replaced the hardcoded `height: 650` with `height: 'calc(100vh - 210px)'` and optimized padding from `60` to `32` to fit perfectly scroll-free in smaller/non-maximized windows.
