...
### Changed
- **Optimized Sidebar & Help Modal Scrollbar Visibility**: Overrode React Native Web's default `<ScrollView>` implementation which forces an absolute `overflow-y: scroll` scrollbar track even when content fits. Explicitly applied an inline `overflowY: 'auto'` style override on the main sidebar navigator (`Sidebar.js`) and the documentation category selector (`HelpModal.js`). This ensures that scrollbars remain completely hidden under standard window scales where all options are visible, showing up automatically only if the window height is resized past threshold limits.
