...
- **Updated Project Design Rules & Style Standards (CONTRIBUTING.md)**: Overhauled the development contribution rules inside `CONTRIBUTING.md` to include a strict "Changelog Formatting & Style Protocol" instructing contributors to write objective, technical, and open-source friendly release logs, while strictly banning subjective marketing jargon (such as "beautiful", "gorgeous", "premium", "sleek", "goldilocks", "sweet spot").
...
- **Source Directory Map**: Added a source directory tree to `CONTRIBUTING.md` so contributors understand the modular file structure.
