...
- **Duplicate Gallery Entries on Download (`src/store.js`)**: Resolved the root cause of videos appearing twice in the gallery after download. The `downloads` queue was being persisted to disk along with history and settings. On every app restart, any queued or paused downloads were restored from disk and re-downloaded by the queue manager, causing `addToHistory` to run again for videos already in the gallery. Fixed by adding a `partialize` filter to the Zustand persist config that excludes `downloads` and other session-only state from disk persistence — only `history`, `playlists`, and `settings` are now written to disk.
